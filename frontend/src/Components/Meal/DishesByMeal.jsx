...
export const DishesByMeal = () => {
    ...
    let params = new URLSearchParams({
     ...
     if( dishes.length === 0){
        return (
          <>
          <div className="max-container2 text-2xl font-semibold mt-10"><h1>{plural(meal && meal.name)}</h1></div>
          <div className='max-container2 flex  flex-col  my-10'>{`There is currently no ${meal && meal.name} dishes listed. We are regularly adding more recipes. Please check back later. If you have a recipe to share, please head to the submit recipe tab and add your recipe for review`} </div>
          </>
        )
      }
