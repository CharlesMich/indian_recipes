...
export const MealTypeByCuisine = () => {
    const cuisineid = useParams().id
    const dispatch = useDispatch()

    const location = useLocation()
    const { cuisineName, cuisineId } = location.state 

    const meals = useSelector(state => Object.values(state.myMeals))

    useEffect(()=> {
      dispatch(fetchAllMeals())
    }, [dispatch, cuisineName, ])
    ...
    if (!meals) {
      ...
      return (
       <div className="flex flex-col min-h-screen  bg-gray-100">
        ...
        <div className="max-container text-2xl font-semibold mt-10"><h1>Explore the culinary world of {cuisineName} cuisine</h1></div>
       <div  className="max-container flex justify-center flex-wrap gap-9 my-10">{meals.map((item, idx)=> (
         <Link to={`/dishesbycuisine/${idx}`} state={{mealId: item.id, cuisineId: cuisineId }} key={idx}><div className="flex sm:w-[300px] 
