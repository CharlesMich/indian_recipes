    ...
    if (!cuisines) {
      ...
      return (
       ...
       <div  className="max-container flex justify-center flex-wrap gap-9 my-10">{cuisines.map((item, idx)=> (
        <Link to={`/dishes/${idx}`} state={{stateId: "notexisting", myMeal: "notexisting", myState: "notexisting", myIngredient: "notexisting", myCuisine:item.id }} key={idx}> <div className="flex sm:w-[300px] 
