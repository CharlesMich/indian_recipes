...
export const MealTypeByState = () => {
  ...
  // we receive the state id here.
    ...
    const { stateName, stateId } = location.state 
    ...
    if (!meal) {
      ...
      return (
       <div className="flex flex-col min-h-screen  bg-gray-100">
        ...
        <div className="max-container text-2xl font-semibold mt-10"><h1>Explore the culinary world of {stateName}</h1></div>
       <div  className="max-container flex justify-center flex-wrap gap-9 my-10">{meals.map((item, idx)=> (
         <Link to={`/dishesbystate/${idx}`} state={{mealId: item.id, stateId: stateId }} key={idx}><div className="flex sm:w-[300px] 
